...
read_files_duckdb <- function(file_type) {
  #create sql statement
  sql_statement <- paste0(
    "CREATE TABLE IF NOT EXISTS ",
    file_type,
    " AS
  SELECT * FROM read_csv('lehd_files/*/*",
    file_type,
    "*.csv.gz',
  filename = TRUE)"
  )
  ...
  # get filename list from the table
  db_files <- tbl(lehd_con, file_type) |>
      select(filename) |>
      distinct() |>
      collect() |>
      pull() |>
  ...
  local_files <- list.files(
      "lehd_files/",
      pattern = paste0(file_type, ".*csv.gz"),
      recursive = TRUE,
      full.names = TRUE
    )
  
  missing_files <- setdiff(local_files, db_files)

  # get column types for the table
  col_types <- dbGetQuery(
    lehd_con,
    paste0("PRAGMA table_info('", file_type, "')")
  ) |>
    filter(name != "filename") |>
    select(name, type) |>
    deframe()

  # append missing files to the table
